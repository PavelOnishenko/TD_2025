...
| Active pass | Pass 2: highest lint-error files. `render.ts` and `ui.ts` are complete for lint errors; continue with `js/core/Game.ts`. |
...
| Next files | Continue Pass 2 with `js/core/Game.ts`, then `js/systems/tutorial.ts`. Keep subpasses small, run `npm run build`, targeted `node --test` coverage for touched behavior, touched-file lint, then `npm run test:neon` and full lint summary before the next checkpoint. |
| Latest commands/results | 2026-06-27 Pass 2 UI buttons/canvas/pause/leaderboard/HUD subpass and UI helper folder move: `npm run build` passed; targeted `node --test test/hudTutorialOverlay.test.js test/tutorial.test.js test/game/*.test.js` passed 71/71 after the folder move; `npm run test:neon` passed 180/180 before the folder move; scoped `npx eslint js/systems/ui.ts js/systems/ui/*.ts --format stylish` reported 0 errors / 36 warnings after the folder move; full `npx eslint js --ext .ts --format json` reported 51 errors / 428 warnings before the folder move. |
| Known blockers | `tsconfig.json` has `noCheck: true`; `Game.ts`, `tutorial.ts`, and the remaining lint-error files still need conservative splits and typed state slices before it can be removed. `ui.ts` is now below the hard file-length limit and warning-only. |
