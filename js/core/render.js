...
function drawGrid(game) {
    ...
    grid.forEach(cell => {
        ...
        if (isPreparationPhase && !cell.occupied) {
            ...
            const pulse = Math.sin(elapsed * 1.8 + offset);
            pulseIntensity = (pulse + 1) / 2;

            const baseRadius = Math.min(cell.w, cell.h) / 2;
            const innerRadius = baseRadius * 0.35;
            const glowRadius = baseRadius * (1.05 + pulseIntensity * 0.08);
            const glowOpacity = 0.05 + pulseIntensity * 0.08;
            ...
            if (typeof ctx.createRadialGradient === 'function') {
                ...
                gradient.addColorStop(0, 'rgba(128, 234, 255, 0.9)');
                gradient.addColorStop(0.55, 'rgba(128, 234, 255, 0.35)');
                gradient.addColorStop(1, 'rgba(128, 234, 255, 0)');
                ctx.fillStyle = gradient;
            } else {
                ctx.fillStyle = 'rgba(128, 234, 255, 0.3)';
            }
