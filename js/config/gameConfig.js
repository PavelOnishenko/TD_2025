export const gameConfig = {
    ...
    tutorial: {
        steps: [
            {
                id: 'build-tower',
                name: 'Build Your First Tower',
                wave: 1,
                highlight: ['world.canvas', 'hud.energy'],
                text: 'Click on a glowing platform cell to build your first tower. Each tower costs 12 energy.',
                image: 'assets/platform.png',
                imageAlt: 'Glowing platform ready for tower construction',
                sound: { label: 'placement.wav', key: 'placement' },
                checkComplete: game => Array.isArray(game?.towers) && game.towers.length > 0,
            },
            {
                id: 'switch-color',
                name: 'Tune Your Crystal',
                wave: 1,
                highlight: ['world.canvas'],
                text: 'Select a tower to switch its color. Matching enemy colors unleashes maximum damage.',
                image: 'assets/tower_1R.png',
                imageAlt: 'Tower prepared to change color',
                sound: { label: 'color_switch.mp3', key: 'colorSwitch' },
                checkComplete: (_, context) => Boolean(context?.events?.colorSwitched),
            },
            {
                id: 'start-wave',
                name: 'Begin the Assault',
                wave: 1,
                highlight: ['hud.nextWave'],
                text: 'Press "Next Wave" when you are ready. Hold the line and protect the base!',
                image: 'assets/bg_anim.gif',
                imageAlt: 'Portal shimmering before the attack',
                sound: { label: 'portal_spawn.mp3', key: 'portalSpawn' },
                checkComplete: (game, context) => Boolean(context?.events?.waveStarted) || Boolean(game?.waveInProgress),
            },
        ],
    },
    projectiles: {
