function getTowerEmitterPosition(tower) {
    ...
    return { x: tower.x + tower.w / 2, y: tower.y + tower.h * baseFactor };
}

function getTowerFlashRadius(tower) {
    return tower.w * 0.28 + (tower.level - 1) * 4;
}

function getTowerFlashColor(tower, intensity) {
    const alpha = 0.6 * intensity;
    return tower.color === 'red' ? `rgba(255,140,100,${alpha})` : `rgba(140,190,255,${alpha})`;
}

export function drawTowerMuzzleFlashIfNeeded(ctx, tower) {
    if (!tower) 
        return;
    ...
    drawTowerMuzzleFlash(tower, ctx, intensity);
}

function drawTowerMuzzleFlash(tower, ctx, intensity) {
...
function getPlacementFlashIntensity(tower) {
    ...
    return Math.max(0, Math.min(1, tower.placementFlashTimer / tower.placementFlashDuration));
}

function getPlacementFlashPalette(tower) {
    const redPalette = { inner: [255, 200, 170], outer: [255, 120, 80] };
    const bluePalette = { inner: [180, 210, 255], outer: [90, 150, 255], };
    if (!tower) {
        return { inner: [255, 255, 255], outer: [255, 255, 255] };
    }
...
export function drawTowerPlacementFlash(ctx, tower) {
    ...
    const { gradient, center, radius } = getPLacementFlashGradient(tower, intensity, ctx);
...
function getPLacementFlashGradient(tower, intensity, ctx) {
    ...
    return { gradient, center, radius };
}

function getTowerGlowProfile(tower, pulse) {
    const levelProfiles = {
        1: { radiusBase: 0.68, radiusPulse: 0.14, coreBase: 0.24, corePulse: 0.16, innerAlphaBase: 0.32, innerAlphaPulse: 0.2, innerRadiusRatio: 0.14, midStop: 0.42 },
        2: { radiusBase: 0.76, radiusPulse: 0.16, coreBase: 0.28, corePulse: 0.18, innerAlphaBase: 0.38, innerAlphaPulse: 0.22, innerRadiusRatio: 0.16, midStop: 0.46 },
        3: { radiusBase: 0.84, radiusPulse: 0.18, coreBase: 0.34, corePulse: 0.22, innerAlphaBase: 0.44, innerAlphaPulse: 0.24, innerRadiusRatio: 0.18, midStop: 0.5 },
    };
    ...
    return { 
                radius, coreAlpha: profile.coreBase + profile.corePulse * pulse, innerColor: palette.inner, 
                outerColor: palette.outer, innerRadiusRatio: profile.innerRadiusRatio, midStop: profile.midStop 
            };
}

function getTowerGlowPalette(tower, innerAlpha) {
    const redPalette = {
        1: { inner: [255, 178, 140], outer: [255, 120, 90] },
        2: { inner: [255, 160, 120], outer: [255, 115, 85] },
        3: { inner: [255, 150, 120], outer: [255, 120, 90] },
    };
    const bluePalette = {
        1: { inner: [170, 210, 255], outer: [110, 160, 255] },
        2: { inner: [160, 200, 255], outer: [108, 170, 255] },
        3: { inner: [150, 190, 255], outer: [110, 160, 255], },
    };
    const paletteByColor = tower.color === 'red' ? redPalette : bluePalette;
    const palette = paletteByColor[tower.level] ?? paletteByColor[3];
    if (!palette) {
        return { inner: `rgba(255,255,255,${innerAlpha})`, outer: 'rgba(255,255,255,0)', };
    }
    return {
...
export function drawTowerTopGlowIfNeeded(ctx, tower) {
    if (!tower) 
        return;
    ...
    drawTowerTopGlow(ctx, x, y, profile);
}

function drawTowerTopGlow(ctx, x, y, profile) {
    const gradient = ctx.createRadialGradient(x, y, profile.radius * profile.innerRadiusRatio, x, y, profile.radius);
