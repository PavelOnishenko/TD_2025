...
export function createGameRuntime(
    ...
    const battleCommandController = new BattleCommandController(
        stateMachine,
        player,
        battleMap,
        turnManager,
    ...
    const battleCoordinator = new GameBattleCoordinator(
        game.input,
        stateMachine,
        {
            player,
    ...
    const worldModeController = new WorldModeController(game.input, player, worldMap, encounterSystem, new ItemDiscoverySplash(), {
        ...
        onStartBattle: (enemies: Skeleton[], terrainType: TerrainType) =>
            stateMachine.transition(MODES.BATTLE, { enemies, terrainType }),
        onAddBattleLog: (m, t = 'system') => hudCoordinator.addBattleLog(m, t),
        onUpdateHUD: () => hudCoordinator.updateHUD(),
        onRememberTraveler: (traveler, disposition) =>
            loreBookController.rememberTraveler(traveler, disposition),
