...
export function getPassiveAvoidChance(actor: CombatParticipant): number {
    const baseAvoid = Math.max(0, Number(actor.avoidChance ?? 0));
    if (!(actor instanceof Skeleton)) {
        return Math.min(0.95, baseAvoid);
    }
    const behaviorAvoid = Math.max(0, Number(actor.behavior?.avoidHitChance ?? 0));
    return Math.min(0.95, baseAvoid + behaviorAvoid);
}
...
export function isHostileIntent(intent: BattleActionIntent | null): boolean {
    return Boolean(intent && ['fast', 'attack', 'heavy-release', 'shoot'].includes(intent.type));
}
