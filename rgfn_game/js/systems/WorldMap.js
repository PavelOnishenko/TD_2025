...
// Fog of war states
const FOG_STATE = {
    UNKNOWN: 'unknown',      // Never visited
    DISCOVERED: 'discovered', // Currently visible (player is here)
    HIDDEN: 'hidden'         // Visited but not currently visible
};
...
export default class WorldMap {
    constructor(columns, rows, cellSize) {
        ...
        this.fogStates = new Map(); // Track fog state for each cell
        this.terrainData = new Map(); // Store terrain data for each cell
    ...
    generateCellTerrain(col, row) {
        ...
        // Choose terrain type
        const terrainTypes = [
            { type: 'grass', color: '#4a7c3e', probability: 0.4 },
            { type: 'forest', color: '#2d5a2d', probability: 0.25 },
            { type: 'mountain', color: '#6b6b6b', probability: 0.15 },
            { type: 'water', color: '#3a6ea5', probability: 0.15 },
            { type: 'desert', color: '#c9a86a', probability: 0.05 }
        ];
    ...
    generateTerrainPattern(seed) {
        // Generate a simple pattern (dots, lines, etc.) for visual variety
        const patterns = ['dots', 'lines', 'cross', 'plain'];
    ...
    discoverCell(col, row) {
        const key = this.getCellKey(col, row);
        const currentState = this.fogStates.get(key);
        ...
        this.grid.forEachCell((cell, c, r) => {
    ...
    drawCell(ctx, cell, fogState, terrain, col, row) {
        switch (fogState) {
    ...
    drawTerrain(ctx, cell, terrain, brightness) {
        if (!terrain) return;
    ...
    drawTerrainPattern(ctx, cell, pattern, brightness) {
