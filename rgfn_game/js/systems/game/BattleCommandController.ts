...
export default class BattleCommandController {
    ...
    public resolveEnemyTurn(enemy: Skeleton): 'continue' | 'defeat' | 'victory' {
        ...
        this.callbacks.onAddBattleLog(`[ROUND] ${enemy.name} decides on ${this.describeIntent(enemyIntent)}.`, 'system');
        if (this.pendingPlayerIntent) {
            this.callbacks.onAddBattleLog(`[ROUND] Player commitment against ${enemy.name}: ${this.describeIntent(this.pendingPlayerIntent)}.`, 'system');
        }
    ...
    private requiresTarget(type: CombatIntentType): boolean {
        return ['fast', 'attack', 'heavy', 'shoot', 'heavy-release'].includes(type);
    }
    ...
    private logPlayerIntent(type: CombatIntentType, direction: BattleActionIntent['direction'], target: Skeleton | null): void {
        ...
        const messages: Record<string, string> = {
            fast: `You twitch into a fast ${direction} strike at ${targetName}.`,
        ...
        this.callbacks.onAddBattleLog(`[ROUND] Queued intent: ${this.describeIntent(this.pendingPlayerIntent ?? { type, direction, targetId: getParticipantId(target), source: 'player' })}.`, 'system');
    }
    ...
    private describeIntent(intent: BattleActionIntent): string {
        const direction = intent.direction ? ` ${intent.direction}` : '';
        return `${intent.type}${direction}`;
    }
