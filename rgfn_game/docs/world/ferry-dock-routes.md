...
## Debugging note (March 31, 2026 follow-up)

A key integration bug was found after initial implementation:

- The keyboard `enterVillage` action path in `WorldModeController.updateWorldMode()` only tried village prompt logic.
- Result: pressing Space on a dock did **not** open ferry prompt even though button-driven world interaction fallback could.

Fix:

- Keyboard action handling now tries village prompt first, then falls back to ferry prompt opening when village entry is not applicable.

Practical takeaway:

- Any new world interaction tied to `enterVillage` must be wired in **both**:
  1. button/runtime interaction flow,
  2. direct keyboard flow inside world mode update loop.
