...
2. A local popup appears on the world map near the village tile with two actions:
   - `Enter`
   - `Pass by`
3. `Enter` transitions to village mode.
...
- Popup is opened by pressing `Space` (or `Enter Village (Space)` button) **while standing on a village tile**.
...
3. Press `Space` -> popup appears near the hero tile.
4. Press `Pass by` -> popup closes, still on world map.
5. Press `Space` again -> popup appears.
6. Press `Enter` -> village mode opens.
